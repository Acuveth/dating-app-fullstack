...
const ConversationHelpersSetup = ({ onUpdate, initialData = {} }) => {
  ...
  const [tempTruths, setTempTruths] = useState({
    truth1: "",
    truth2: "",
    lie: "",
  });
  ...
  const openTwoTruthsModal = () => {
    ...
    setTempTruths({ truth1: "", truth2: "", lie: "" });
    setModalVisible(true);
  };
  ...
  const addIceBreakerAnswer = () => {
    ...
    if (selectedHelpers.iceBreakerAnswers.length >= 3) {
      Alert.alert(
        "Limit Reached",
        "You can only add up to 3 ice breaker answers"
      );
      return;
    }
  ...
  const addWouldYouRatherAnswer = () => {
    ...
    if (selectedHelpers.wouldYouRatherAnswers.length >= 3) {
      Alert.alert(
        "Limit Reached",
        "You can only add up to 3 Would You Rather answers"
      );
      return;
    }

    const questionObj = questions.wouldYouRatherQuestions.find(
      (q) => `${q.option1} OR ${q.option2}` === tempQuestion
    );

    setSelectedHelpers((prev) => ({
      ...prev,
      wouldYouRatherAnswers: [
        ...prev.wouldYouRatherAnswers,
        {
  ...
  const addTwoTruthsOneLie = () => {
    ...
    if (selectedHelpers.twoTruthsOneLie.length >= 2) {
      Alert.alert(
        "Limit Reached",
        "You can only add up to 2 Two Truths and a Lie sets"
      );
      return;
    }
            ...
            {modalType === "twotruths" && (
              <>
                ...
                <TextInput
                  ...
                  onChangeText={(text) =>
                    setTempTruths((prev) => ({ ...prev, truth1: text }))
                  }
                  maxLength={100}
                />
                ...
                <TextInput
                  ...
                  onChangeText={(text) =>
                    setTempTruths((prev) => ({ ...prev, truth2: text }))
                  }
                  maxLength={100}
                />

                <Text style={styles.inputLabel}>Statement 3:</Text>
                <TextInput
                  style={styles.input}
                  placeholder="Enter a statement..."
                  placeholderTextColor="#666"
                  ...
                  onChangeText={(text) =>
                    setTempTruths((prev) => ({ ...prev, lie: text }))
                  }
                  maxLength={100}
                />
              </>
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>
        Add some fun questions to help break the ice!
      </Text>
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>
            Ice Breakers ({selectedHelpers.iceBreakerAnswers.length}/3)
          </Text>
          <TouchableOpacity
            style={styles.addBtn}
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>
            Would You Rather ({selectedHelpers.wouldYouRatherAnswers.length}/3)
          </Text>
          <TouchableOpacity
            style={styles.addBtn}
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>
            Two Truths and a Lie ({selectedHelpers.twoTruthsOneLie.length}/2)
          </Text>
          <TouchableOpacity
            style={styles.addBtn}
        ...
        {selectedHelpers.twoTruthsOneLie.map((item, index) => (
          <View key={index} style={styles.helperItem}>
            <Text style={styles.statementText}>• {item.truth1}</Text>
            <Text style={styles.statementText}>• {item.truth2}</Text>
            <Text style={styles.statementText}>• {item.lie}</Text>
            <TouchableOpacity
              style={styles.removeBtn}
